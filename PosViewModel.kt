...
class PosViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun confirmPixPaymentInTerminal() {
        val order = _currentOrder.value ?: return
        viewModelScope.launch {
            val result = terminalManager.simulatePixConfirm(order.totalCents, order.tableOrCommandName)
            repository.recordPaymentAndCompleteOrder(
                orderId = order.id,
    ...
    fun openCashierShift(initialFloatCents: Long) {
        viewModelScope.launch {
            repository.openCashierShift(initialFloatCents)
        }
    }
    ...
    fun addNewProduct(name: String, categoryId: Long, priceCents: Long, description: String, code: String) {
        viewModelScope.launch {
