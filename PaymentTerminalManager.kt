...
sealed class TerminalStep {
    ...
    data class WaitingForCard(val brand: TerminalBrand, val amountCents: Long, val method: PaymentMethod) : TerminalStep()
    data class DisplayingPixQr(val amountCents: Long, val qrCodePayload: String) : TerminalStep()
...
class PaymentTerminalManager {
    ...
    suspend fun startPayment(
        ...
        if (method == PaymentMethod.PIX) {
            val pixPayload = "00020126580014br.gov.bcb.pix0136${UUID.randomUUID()}5204000053039865405${String.format(java.util.Locale.US, "%.2f", amountCents / 100.0)}5802BR5915PDV_GOURMET6009SAO_PAULO62070503***6304"
            _currentStep.value = TerminalStep.DisplayingPixQr(amountCents, pixPayload)
        ...
        // Card flow
        _currentStep.value = TerminalStep.WaitingForCard(brand, amountCents, method)
        delay(1200)
        ...
        val receiptLines = generateReceiptLines(tableOrCommandName, amountCents, method, installments, chosenBrand, nsu, authCode)
    ...
    suspend fun simulatePixConfirm(amountCents: Long, tableOrCommandName: String): PaymentResult.Success {
        val nsu = Random.nextInt(100000000, 999999999).toString()
        ...
        val receiptLines = generateReceiptLines(tableOrCommandName, amountCents, PaymentMethod.PIX, 1, "PIX BANCO CENTRAL", nsu, authCode)
